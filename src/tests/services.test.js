...
tape('handleFileCreation', async (t) => {
    ...
    t.equal(renderSpy.calledWith('template', {}), true, 'should format the template')
})
...
tape('tasksHandler', async (t) => {
    ...
    await Stub([{type: TASKS.CREATE,file: {path: '', template: ''}, variables: {}}])(0);
    await Stub([{type: 'NO_TASK',file: {path: '', template: ''}, variables: {}}])(0);
